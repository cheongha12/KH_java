	...
	public void testInetAddressEx() {
		...
		try {
			...
			for (int i = 0; i < localIpArr.length; i++) {
				if (localIpArr[i] < 0) {
					System.out.println(localIpArr[i] + 256);
				}

				else {

					System.out.println(localIpArr[i]);
				}
			...
			byte[] byteArr = new byte[] { (byte) 223, (byte) 130, (byte) 200, 104 };
			System.out.println(Arrays.toString(byteArr));
			InetAddress byte2Arr = InetAddress.getByAddress(byteArr);
			System.out.println(byte2Arr); // 223.130.195.12
			...
			System.out.println(Arrays.toString(ipAll)); // www.naver.com/223.130.195
